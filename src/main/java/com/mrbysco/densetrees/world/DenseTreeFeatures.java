...
public class DenseTreeFeatures {
	...
	public static final RegistryObject<ConfiguredFeature<?, ?>> DENSE_JUNGLE_TREE = CONFIGURED_FEATURES.register("dense_jungle_tree", () -> new ConfiguredFeature<>(Feature.TREE, createJungleTree().decorators(ImmutableList.of(new CocoaDecorator(0.2F), TrunkVineDecorator.INSTANCE, LeaveVineDecorator.INSTANCE)).ignoreVines().build()));
	...
	public static final RegistryObject<ConfiguredFeature<?, ?>> DENSE_SWAMP_OAK = CONFIGURED_FEATURES.register("dense_swamp_oak", () -> new ConfiguredFeature<>(Feature.TREE, createStraightBlobTree(DenseRegistry.DENSE_OAK_LOG.get(), Blocks.OAK_LEAVES, 5, 3, 0, 3).decorators(ImmutableList.of(LeaveVineDecorator.INSTANCE)).build()));
